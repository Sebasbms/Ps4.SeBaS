...
@ini_set('memory_limit', '512M'); // Límite seguro para Termux/KSWEB
...
if (is_dir($storage_dir)) {
    ...
    foreach ($carpetas as $carpeta) {
        // Formato típico MicroSD en Android: 1A2B-3C4D
    ...
    // Si quitaron la SD, borramos el túnel viejo
    if (!$sd_encontrada) { @unlink($microsd_link); }
}

// 3. DETECCIÓN DE IP LOCAL (Ya viene optimizado de tu versión)
function getLocalIP() {
    ...
    if (strpos(strtoupper(PHP_OS), 'WIN') === 0) {
        ...
        foreach ($out as $line) {
            if (preg_match('/inet\s+([0-9\.]+)/i', $line, $matches)) {
                if ($matches[1] != '127.0.0.1' && strpos($matches[1], '127.') !== 0) { $ip = $matches[1]; if (strpos($ip, '192.168.') === 0) break; }
            }
        }
    }
    return $ip;
...
// 5. OBTENER LISTA DE JUEGOS RPI (UNIFICADA: Interna + MicroSD)
            ...
            foreach ($files as $file) {
                if ($file !== '.' && $file !== '..' && strtolower(pathinfo($file, PATHINFO_EXTENSION)) === 'pkg') {
                    ...
                    $pkgs[] = [
                        ...
                        'size' => $size, 
                        'size_fmt' => $size_fmt,
                        'origen' => $ruta['tipo'] // Enviamos a la interfaz visual el origen
                    ];
                }
            }
        }
...
// 6. EXTRACTOR BINARIO SECUENCIAL PKG (Adaptado para buscar en SD)
if (isset($_GET['extract_pkg'])) {
    header('Content-Type: application/json');
    $file = basename($_GET['extract_pkg']);
    
    // Verificamos si el archivo está en la SD o en la Interna
    ...
    if (file_exists($path)) {
        ...
        if ($fp) {
            ...
            if ($sfo_pos !== false) {
                ...
                if (preg_match('/(CUSA\d{5})/i', $sfo_data, $m)) { $meta['cusa'] = strtoupper($m[1]); }
            }
...
<head>
    ...
    <style>
        ...
        /* SISTEMA DE TEMAS DINÁMICOS AMPLIADO */
        ...
        /* ==========================================
           NUEVOS TEMAS NEUMORPHISM (BLANCO Y NEGRO)
           ========================================== */
        ...
        /* SOBRESCRITURAS NEUMORPHISM (Mantiene tu diseño intacto) */
        ...
        body[data-theme^="neumorphism"] .bg-black\/60,
        body[data-theme^="neumorphism"] .bg-black\/50,
        body[data-theme^="neumorphism"] .bg-black\/40,
        body[data-theme^="neumorphism"] .bg-white\/5,
        body[data-theme^="neumorphism"] .bg-white\/10 {
            background: var(--neu-surface) !important; box-shadow: var(--neu-shadow) !important; border: 1px solid var(--neu-border) !important; backdrop-filter: none !important; color: var(--text-main);
        }
        body[data-theme^="neumorphism"] input,
        body[data-theme^="neumorphism"] .shadow-inner,
        body[data-theme^="neumorphism"] .game-card {
            background: var(--neu-surface) !important; box-shadow: var(--neu-shadow-inset) !important; border: none !important;
        }
        body[data-theme^="neumorphism"] .text-white { color: var(--text-main) !important; }
        body[data-theme^="neumorphism"] .text-white\/80,
        body[data-theme^="neumorphism"] .text-white\/70,
        body[data-theme^="neumorphism"] .text-white\/60,
        body[data-theme^="neumorphism"] .text-white\/50,
        body[data-theme^="neumorphism"] .text-white\/40,
        body[data-theme^="neumorphism"] .text-white\/30 { color: var(--text-muted) !important; }
        ...
        /* Estilos del Visor de Capturas (Lightbox) */
...
<body data-theme="cyberpunk">
    ...
    <div id="app-ui">
        ...
        <div class="w-full max-w-md md:max-w-4xl mx-auto px-5 relative z-10 mb-1.5 flex gap-2 shrink-0">
            <div class="flex-1 flex items-center px-4 bg-black/60 rounded-[1.2rem] border border-white/5 focus-within:border-[var(--theme-prim)] transition-all shadow-inner h-[42px] backdrop-blur-md">
                ...
                <input type="text" id="host-ip" value="<?php echo isset($ip_servidor) ? $ip_servidor : ''; ?>" placeholder="192.168.x.x" data-i18n-placeholder="ip_placeholder" class="bg-transparent w-full text-xs font-mono outline-none text-white placeholder-white/20" autocomplete="off">
            ...
            <button onclick="connectManualIP()" class="w-[42px] h-[42px] rounded-[1.2rem] bg-black/60 backdrop-blur-md hover:bg-white/10 border border-white/5 flex items-center justify-center text-white active:scale-95 shrink-0 transition-colors shadow-inner"><i class="fa-solid fa-plug text-sm" style="color: var(--theme-prim);" id="connect-icon"></i></button>
        ...
        <div class="w-full max-w-md md:max-w-4xl mx-auto px-5 relative z-10 mb-2 shrink-0">
            <p id="global-status" class="text-[9px] font-bold text-center font-mono text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.8)] tracking-widest hidden"><i class="fa-solid fa-satellite-dish fa-fade mr-2 text-white"></i> <span id="scan-text">BUSCANDO IP...</span></p>
        </div>
        ...
        <main class="w-full relative z-10 flex-1">
            ...
            <div id="tab-biblioteca" class="tab-content active">
                <div class="flex gap-2 w-full mb-2 shrink-0">
                    <div class="relative flex-1 h-[42px]">
                        <i class="fa-solid fa-search absolute left-4 top-1/2 transform -translate-y-1/2 text-white/30 text-[11px]"></i>
                        <input type="text" id="buscador-biblio" onkeyup="buscarEnBiblioteca(this.value)" placeholder="Buscar juego o app..." data-i18n-placeholder="search_placeholder" class="w-full h-full bg-black/60 backdrop-blur-md border border-white/5 rounded-[1.2rem] pl-10 pr-4 text-xs font-bold tracking-wider text-white outline-none focus:border-[var(--theme-prim)] transition-colors placeholder-white/30 shadow-inner" autocomplete="off">
                    </div>
                    <button onclick="cambiarOrden()" class="w-[42px] h-[42px] rounded-[1.2rem] border border-white/5 bg-black/60 backdrop-blur-md flex items-center justify-center text-white active:scale-95 shrink-0 transition-colors hover:bg-white/5 group shadow-inner">
                        <i id="icono-orden" class="fa-solid fa-arrow-down-short-wide text-sm group-hover:text-white transition-colors" style="color: var(--theme-prim); filter: drop-shadow(0 0 5px color-mix(in srgb, var(--theme-prim) 30%, transparent));"></i>
                    </button>
                </div>
                ...
                <div class="shrink-0 w-full pt-1">
                    <div class="flex justify-between items-end mb-3">
                        <div class="flex flex-col">
                            <h1 class="text-3xl font-black text-white tracking-wide leading-none mb-1.5 drop-shadow-[0_0_8px_rgba(0,0,0,0.8)]" data-i18n="tab_biblio">Biblioteca</h1>
                            <div class="flex items-center gap-2 text-[10px]">
                                ...
                                <span class="text-white/80 font-mono tracking-widest uppercase drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]" data-i18n="titles_installed">TÍTULOS INSTALADOS</span>
                            </div>
                        </div>
            ...
            <div id="tab-ftp" class="tab-content">
                <h1 class="text-3xl font-black mb-1 text-white tracking-wide drop-shadow-[0_0_8px_rgba(0,0,0,0.8)]" data-i18n="tab_transfer">Transferir</h1>
                <p class="text-xs text-white/80 mb-6 font-light tracking-wide drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]" data-i18n="desc_transfer">Envía juegos o aplicaciones a tu consola.</p>
                ...
                <div id="box-mode-rpi" class="glass-panel rounded-[2rem] p-6 mb-6 shrink-0">
                    ...
                    <div id="rpi-pkg-list" class="grid grid-cols-2 md:grid-cols-3 gap-3"></div>
                </div>
                ...
                <form id="ftp-form" onsubmit="enviarArchivoChunks(event)" class="hidden shrink-0">
                    <div class="glass-panel rounded-[2rem] p-6 mb-6">
                        ...
                        <div id="add-path-ui" class="hidden mb-6 flex gap-2">
                            <input type="text" id="new-path-input" placeholder="/ruta/" class="flex-1 bg-black/40 border border-white/10 rounded-xl px-4 text-sm font-mono text-white outline-none focus:border-[var(--theme-prim)]">
                        ...
                        <div onclick="document.getElementById('file-upload').click()" class="w-full aspect-video border border-dashed rounded-3xl flex flex-col items-center justify-center cursor-pointer bg-black/40 hover:bg-black/60 transition-colors group" style="border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent);">
                            ...
                            <span id="file-name-display" class="text-[10px] font-black tracking-widest text-white/60 text-center px-4" data-i18n="touch_select">TOCA PARA SELECCIONAR</span>
            ...
            <div id="tab-icons" class="tab-content">
                <h1 class="text-3xl font-black mb-1 text-white tracking-wide drop-shadow-[0_0_8px_rgba(0,0,0,0.8)]" data-i18n="tab_mod">Modding</h1>
                <p class="text-xs text-white/80 mb-6 font-light tracking-wide drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]" data-i18n="desc_mod">Personaliza el arte de tu biblioteca.</p>
                
                <div id="config-carpeta-iconos" class="glass-panel rounded-[2rem] p-5 mb-6 flex justify-between items-center shrink-0">
                    <div class="flex items-center gap-3 overflow-hidden">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center border shrink-0" style="background-color: color-mix(in srgb, var(--theme-prim) 20%, transparent); border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent);">
                            <i class="fa-solid fa-folder-tree text-lg" style="color: var(--theme-prim);"></i>
                        </div>
                        <div class="flex flex-col overflow-hidden">
                            <span class="text-[9px] font-black tracking-widest uppercase" style="color: var(--theme-prim);">Ruta de Íconos</span>
                            <span id="ruta-actual-display" class="text-[11px] text-white/70 font-mono truncate">htdocs/iconos/ (Defecto)</span>
                        </div>
                    </div>
                    <button type="button" onclick="abrirExploradorIconos()" class="ml-2 shrink-0 bg-white/10 hover:bg-white/20 text-white rounded-xl px-4 py-2.5 text-[9px] font-black tracking-widest transition-colors border border-white/10">
                        CAMBIAR
                    </button>
                </div>

                <div id="explorador-ui" class="hidden glass-panel border rounded-[2rem] p-6 mb-6 shadow-2xl shrink-0 animate-fade-in" style="border-color: color-mix(in srgb, var(--theme-prim) 40%, transparent);">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-sm font-black text-white tracking-widest"><i class="fa-solid fa-folder-open mr-2" style="color: var(--theme-prim);"></i>SELECCIONAR CARPETA</h2>
                        <button type="button" onclick="cerrarExploradorIconos()" class="text-white/50 hover:text-red-400 p-1 transition-colors"><i class="fa-solid fa-xmark text-lg"></i></button>
                    </div>
                    
                    <div class="bg-black/50 p-3 rounded-lg text-[10px] font-mono mb-4 text-green-400 break-all border border-white/5 shadow-inner" id="ruta-actual-explorador">
                        Cargando...
                    </div>

                    <div id="lista-carpetas" class="space-y-2 max-h-48 overflow-y-auto pr-2 mb-6 text-sm custom-scrollbar">
                        <p class="text-center text-white/50 py-4"><i class="fa-solid fa-spinner fa-spin mr-2"></i>Cargando...</p>
                    </div>

                    <button type="button" onclick="guardarRutaIconos()" class="w-full text-black font-black py-3 rounded-xl transition-all text-[10px] tracking-widest active:scale-95" style="background-color: var(--theme-prim); box-shadow: 0 0 15px color-mix(in srgb, var(--theme-prim) 40%, transparent);">
                        GUARDAR AQUÍ PARA ÍCONOS
                    </button>
                </div>
                    ...
                    <div class="glass-panel rounded-[2rem] p-6 mb-6">
                        <div class="flex gap-2 mb-6 items-end shrink-0">
                            <div class="flex-1 relative">
                                ...
                                <input type="text" id="icon-cusa" placeholder="CUSA12345" oninput="this.value = this.value.toUpperCase();" class="w-full bg-black/60 border rounded-2xl px-5 py-4 font-mono text-center text-lg outline-none text-white uppercase shadow-inner backdrop-blur-md" style="border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent);">
                            </div>
                            <button type="button" onclick="respaldarOriginal()" class="h-[58px] w-[58px] bg-black/60 border hover:bg-white/10 text-white/70 hover:text-white rounded-2xl flex items-center justify-center active:scale-95 shrink-0 transition-colors backdrop-blur-md" style="border-color: color-mix(in srgb, var(--theme-prim) 50%, transparent);">
                        ...
                        <div class="flex gap-1.5 p-1 bg-black/40 rounded-[1.2rem] mb-6 border border-white/5 overflow-x-auto custom-scrollbar shrink-0">
                            <button type="button" id="btn-src-gallery" class="flex-1 py-3 px-3 text-[9px] font-black rounded-xl bg-white text-black shadow-lg whitespace-nowrap" onclick="switchIconSource('gallery')" data-i18n="gallery">GALERÍA</button>
                            <button type="button" id="btn-src-backup" class="flex-1 py-3 px-3 text-[9px] font-black rounded-xl text-white/50 whitespace-nowrap" onclick="switchIconSource('backup')" data-i18n="backups">BACKUPS</button>
                            <button type="button" id="btn-src-import" class="flex-1 py-3 px-3 text-[9px] font-black rounded-xl text-white/50 whitespace-nowrap" onclick="switchIconSource('import')" data-i18n="import">IMPORTAR</button>
                            <button type="button" id="btn-src-local" class="flex-1 py-3 px-3 text-[9px] font-black rounded-xl text-white/50 whitespace-nowrap" onclick="switchIconSource('local')" data-i18n="local">LOCAL</button>
                        </div>
                        ...
                        <div id="box-src-import" class="hidden animate-fade-in shrink-0">
                            <div class="bg-black/40 border border-white/5 rounded-[1.5rem] p-5 text-center h-[250px] flex flex-col justify-center">
                                ...
                                <input type="url" id="import-url" placeholder="https://..." class="w-full bg-black/60 rounded-xl px-4 py-3 border font-mono text-xs text-white outline-none mb-4 shadow-inner" style="border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent);">
                        ...
                        <div id="box-src-local" class="hidden animate-fade-in shrink-0">
                            <div onclick="document.getElementById('icon-file').click()" class="w-full h-[250px] bg-black/40 rounded-[1.5rem] border border-dashed flex flex-col items-center justify-center cursor-pointer relative overflow-hidden group hover:bg-black/60 transition-colors" style="border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent);">
                                ...
                                <span id="icon-file-name" class="text-[10px] font-black tracking-widest text-white/50 text-center px-4 z-10" data-i18n="touch_image">TOCA PARA BUSCAR IMAGEN</span>
            ...
            <div id="tab-explorer" class="tab-content">
                <div class="flex justify-between items-end mb-6 shrink-0">
                    <div>
                        <h1 class="text-3xl font-black mb-1 text-white tracking-wide drop-shadow-[0_0_8px_rgba(0,0,0,0.8)]" data-i18n="tab_exp">Explorador</h1>
                        <p class="text-xs text-white/80 font-light tracking-wide drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]" data-i18n="desc_exp">Gestor de archivos interno.</p>
                    </div>
                    <div class="flex gap-1.5 flex-wrap justify-end">
                        <button onclick="addCurrentPathToShortcuts()" class="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-yellow-500/20 hover:text-yellow-400 transition-colors hover:border-yellow-500/50 backdrop-blur-md"><i class="fa-solid fa-star text-[11px]"></i></button>
                        <button onclick="promptCreateFolder()" class="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white transition-colors hover:bg-white/20 backdrop-blur-md"><i class="fa-solid fa-folder-plus text-[11px]"></i></button>
                        <button id="btn-select-mode" onclick="toggleSelectMode()" class="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white transition-colors backdrop-blur-md"><i class="fa-solid fa-list-check text-[11px]"></i></button>
                        <button onclick="if(currentExplorerPath) loadExplorerPath(currentExplorerPath)" class="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white transition-colors hover:bg-white/20 backdrop-blur-md"><i class="fa-solid fa-rotate-right text-[11px]"></i></button>
                ...
                <div id="clipboard-panel" class="hidden mb-4 glass-panel rounded-xl p-3 flex justify-between items-center border shrink-0" style="border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent); box-shadow: 0 0 15px color-mix(in srgb, var(--theme-prim) 10%, transparent);">
                    <div class="flex items-center gap-3 overflow-hidden pr-2">
                        <i class="fa-solid fa-scissors text-sm" style="color: var(--theme-prim);"></i><span id="clipboard-text" class="text-[11px] text-white font-mono truncate">...</span>
                    </div>
                ...
                <div id="multi-action-panel" class="hidden mb-4 glass-panel rounded-xl p-3 flex justify-between items-center border-white/20 animate-fade-in shrink-0">
                    <div class="flex items-center gap-2">
                        ...
                        <span id="multi-select-count" class="text-[10px] text-white font-black tracking-widest uppercase">0 seleccionados</span>
                    </div>
                    <div class="flex gap-2">
                ...
                <div class="glass-panel rounded-[2rem] overflow-hidden flex flex-col h-[55vh] shrink-0 border border-white/10">
                    <div class="bg-black/60 backdrop-blur-md px-5 py-4 border-b flex items-center gap-3 shrink-0" style="border-color: color-mix(in srgb, var(--theme-prim) 20%, transparent);"><i class="fa-solid fa-hard-drive" style="color: color-mix(in srgb, var(--theme-prim) 50%, transparent);"></i><span id="explorer-path-text" class="text-xs font-mono text-white/90 drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]">/</span></div>
                    ...
                    <div id="explorer-list" class="flex-1 p-2 flex flex-col gap-1 overflow-y-auto custom-scrollbar bg-black/20"><div class="text-center text-white/50 text-[10px] font-black tracking-widest uppercase py-20 drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]" data-i18n="config_ip">Configura tu IP.</div></div>
                </div>
            </div>

            <div id="tab-payloads" class="tab-content">
                <h1 class="text-3xl font-black mb-1 text-white tracking-wide drop-shadow-[0_0_8px_rgba(0,0,0,0.8)]" data-i18n="tab_pay">Payloads</h1>
                <p class="text-xs text-white/80 mb-6 font-light tracking-wide drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]" data-i18n="desc_pay">Inyección de código BinLoader.</p>
                <form id="payload-form" onsubmit="enviarPayload(event)">
                    <div class="glass-panel rounded-[2rem] p-6 mb-6 shrink-0">
                        <div class="flex items-center justify-between bg-black/60 border rounded-2xl px-5 py-4 mb-6 shadow-inner backdrop-blur-md" style="border-color: color-mix(in srgb, var(--theme-prim) 20%, transparent);">
                            ...
                            <input type="number" id="payload-port" value="9020" class="bg-transparent text-right font-mono text-white text-lg outline-none w-20 border-b border-dashed focus:border-[var(--theme-prim)]" style="border-color: color-mix(in srgb, var(--theme-prim) 40%, transparent);" required>
                        </div>
                        <div class="flex gap-2 p-1 bg-black/40 rounded-[1.2rem] border border-white/5 backdrop-blur-md">
                            <button type="button" id="btn-pay-gallery" class="flex-1 py-3 text-[9px] font-black tracking-widest rounded-xl bg-white text-black shadow-lg" onclick="switchPayloadSource('gallery')" data-i18n="gallery">GALERÍA</button>
                            <button type="button" id="btn-pay-local" class="flex-1 py-3 text-[9px] font-black tracking-widest rounded-xl text-white/50 transition-colors" onclick="switchPayloadSource('local')" data-i18n="device">DISPOSITIVO</button>
                        </div>
                        ...
                        <div id="box-pay-local" class="mt-4 hidden animate-fade-in shrink-0">
                            <div onclick="document.getElementById('payload-file').click()" class="w-full h-[180px] bg-black/40 rounded-[1.5rem] border border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-black/60 transition-colors group backdrop-blur-md" style="border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent);">
                                ...
                                <span id="payload-name-display" class="text-[10px] font-black tracking-widest text-white/50 px-6 text-center" data-i18n="touch_bin">TOCA PARA BUSCAR .BIN</span>
            ...
            <div id="tab-settings" class="tab-content pt-2">
                <div class="flex items-center justify-between mb-6 shrink-0">
                    <div>
                        <h2 class="text-3xl font-black text-white tracking-wider drop-shadow-[0_0_8px_rgba(0,0,0,0.8)]" data-i18n="tab_set">Ajustes</h2>
                        <p class="text-white/80 text-xs font-light tracking-wide mt-1 drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]" data-i18n="desc_set">Configuración del sistema y visual.</p>
                    </div>
                </div>
                ...
                <div class="flex flex-col w-full glass-panel rounded-3xl p-4 shrink-0 shadow-[0_15px_30px_rgba(0,0,0,0.8)] border-white/10">
                    ...
                    <div class="flex flex-col py-4 border-b border-white/5">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-xs font-black text-white tracking-widest uppercase" data-i18n="set_lang_title">Idioma</span>
                                    <span class="text-[9px] text-white/50" data-i18n="set_lang_desc">Cambia el idioma.</span>
                                </div>
                            </div>
                            <div class="flex bg-black/60 p-1 rounded-xl border border-white/5 shadow-inner">
                                ...
                                <button id="btn-lang-en" onclick="setLanguage('en')" class="px-4 py-2 text-[10px] font-black rounded-lg text-white/50 hover:text-white transition-all">EN</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between py-4 border-b border-white/5 cursor-pointer hover:bg-white/5 rounded-xl px-2 transition-colors -mx-2" onclick="abrirBovedaGlobal()">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col">
                                <span class="text-xs font-black text-white tracking-widest uppercase">Bóveda de Capturas</span>
                                <span class="text-[9px] text-white/50">Ver fotos de todos los juegos.</span>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col py-4 border-b border-white/5">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-xs font-black text-white tracking-widest uppercase">Wallpaper</span>
                                    <span class="text-[9px] text-white/50">Fondo y difuminado (Blur).</span>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <button onclick="document.getElementById('custom-bg-upload').click()" class="w-8 h-8 rounded-xl bg-black/50 flex items-center justify-center hover:text-black transition-all border active:scale-95 shrink-0" style="color: var(--theme-prim); border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent);"><i class="fa-solid fa-upload text-sm"></i></button>
                        ...
                        <div class="flex items-center gap-4 pl-12 pr-2">
                            <i class="fa-solid fa-droplet text-white/30 text-[10px]"></i>
                            <input type="range" id="blur-slider" min="0" max="15" step="1" value="0" class="w-full h-1.5 bg-black/60 rounded-lg appearance-none cursor-pointer border border-white/5 shadow-inner">
                    ...
                    <div class="flex flex-col py-4 border-b border-white/5">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-xs font-black text-white tracking-widest uppercase">Partículas</span>
                                    <span class="text-[9px] text-white/50">Efecto polvo estelar.</span>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center gap-4 pl-12 pr-2">
                            <i class="fa-solid fa-minus text-white/30 text-[10px]"></i>
                            <input type="range" id="particles-slider" min="50" max="500" step="50" value="400" class="w-full h-1.5 bg-black/60 rounded-lg appearance-none cursor-pointer border border-white/5 shadow-inner">
                    ...
                    <div class="flex items-center justify-between py-4 border-b border-white/5">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col">
                                <span class="text-xs font-black text-white tracking-widest uppercase" data-i18n="set_wake_title">Pantalla Activa</span>
                                <span class="text-[9px] text-white/50" data-i18n="set_wake_desc">Evita que se apague.</span>
                            </div>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <div class="flex items-center justify-between py-4 border-b border-white/5">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col">
                                <span class="text-xs font-black text-white tracking-widest uppercase" data-i18n="set_notif_title">Notificaciones</span>
                                <span class="text-[9px] text-white/50" data-i18n="set_notif_desc">Alertas flotantes.</span>
                            </div>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <div class="flex items-center gap-4 w-full">
                            ...
                            <div class="flex flex-col flex-1 pr-4">
                                <div class="flex justify-between items-center mb-2">
                                    <div>
                                        <span class="text-xs font-black text-white block tracking-widest uppercase" data-i18n="set_vol_title">Volumen / Sonidos</span>
                                        <span class="text-[9px] text-white/50" data-i18n="set_vol_desc">Ajusta la intensidad.</span>
                                    </div>
                                ...
                                <input type="range" id="volume_slider" min="0" max="1" step="0.1" value="0.5" class="w-full h-1.5 bg-black/60 rounded-lg appearance-none cursor-pointer border border-white/5 shadow-inner" onchange="AudioEngine.updateVol(this.value)">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between py-4 mb-2">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col">
                                <span class="text-xs font-black text-white tracking-widest uppercase" data-i18n="set_temp_title">Limpiar Temporales</span>
                                <span class="text-[9px] text-white/50" data-i18n="set_temp_desc">Libera caché del servidor.</span>
                            </div>
                        </div>
                        <button onclick="limpiarTemporales()" class="w-10 h-10 rounded-xl bg-black/50 text-red-400 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all border border-red-500/30 active:scale-95 shrink-0">
                ...
                <div class="flex flex-col mt-6 mb-4 glass-panel rounded-3xl p-4 shadow-[0_15px_30px_rgba(0,0,0,0.8)] border-white/10">
                    <div class="flex items-center gap-4 mb-3">
                        ...
                        <div class="flex flex-col">
                            <span class="text-xs font-black text-white tracking-widest uppercase">Tema Visual</span>
                            <span class="text-[9px] text-white/50">Desliza para elegir.</span>
                        </div>
                    </div>
                    <div class="flex gap-2 overflow-x-auto custom-scrollbar pb-2 px-1" id="theme-scroll-container">
                        <button onclick="changeTheme('cyberpunk', this)" class="theme-btn active bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">CYBERPUNK</button>
                        <button onclick="changeTheme('ps5', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">PS5 CLASSIC</button>
                        
                        <button onclick="changeTheme('neumorphism-light', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">NEUMORPHISM LIGHT</button>
                        <button onclick="changeTheme('neumorphism-dark', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">NEUMORPHISM DARK</button>

                        <button onclick="changeTheme('spiderman', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">SPIDER-MAN</button>
                        <button onclick="changeTheme('batman', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">BATMAN</button>
                        <button onclick="changeTheme('gow', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">GOD OF WAR</button>
                        <button onclick="changeTheme('bloodborne', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">BLOODBORNE</button>
                        <button onclick="changeTheme('hollowknight', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">HOLLOW KNIGHT</button>
                        <button onclick="changeTheme('got', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">GHOST OF TSUSHIMA</button>
                        <button onclick="changeTheme('matrix', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">THE MATRIX</button>
                        <button onclick="changeTheme('vicecity', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">GTA VICE CITY</button>
                        <button onclick="changeTheme('evangelion', this)" class="theme-btn bg-white/5 border border-white/10 text-white/50 hover:text-white text-[9px] font-black py-2 px-4 rounded-lg transition-all shrink-0 whitespace-nowrap">EVANGELION</button>
                    </div>
                </div>

                <div class="flex flex-col items-center justify-center mt-6 mb-10 opacity-60 shrink-0">
                    <i class="fa-brands fa-playstation text-[3.5rem] mb-4 text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]"></i>
                    <span class="text-[12px] font-black tracking-widest text-white mb-1 footer-sig drop-shadow-[0_0_2px_rgba(0,0,0,0.8)]">GoldHen Manager v2.1</span>
        ...
        <div class="floating-btn-global floating-hidden" id="floating-btn-rpi">
            <button onclick="iniciarColaInstalacionRPI()" class="w-full btn-ps5-primary !bg-[linear-gradient(135deg,#fbbf24,#d97706)] !text-white !border-yellow-400/30 !shadow-[0_0_25px_rgba(251,191,36,0.5),inset_0_2px_5px_rgba(255,255,255,0.3)] rounded-2xl py-5 font-black tracking-widest text-[10px] flex items-center justify-center gap-3"><i class="fa-solid fa-download text-xl"></i> <span id="rpi-install-text" data-i18n="btn_install_ps4">INSTALAR EN PS4</span></button>
        </div>
    ...
    <div id="custom-modal" class="fixed inset-0 bg-[#05050a]/90 backdrop-blur-md z-[100] hidden flex items-center justify-center transition-opacity opacity-0 px-4">
        <div id="modal-card" class="bg-black/80 backdrop-blur-lg border p-8 rounded-[2rem] w-full max-w-sm text-center transform scale-95 transition-transform relative overflow-hidden glass-panel" style="border-color: color-mix(in srgb, var(--theme-prim) 30%, transparent); box-shadow: 0 0 40px color-mix(in srgb, var(--theme-prim) 15%, transparent);">
            <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[var(--theme-prim)] via-white to-[var(--theme-prim)]"></div>
    ...
    <div id="ps5-dialog" class="fixed inset-0 bg-[#05050a]/90 backdrop-blur-md z-[110] hidden flex items-center justify-center transition-opacity opacity-0 px-4">
        <div id="ps5-dialog-card" class="bg-black/80 backdrop-blur-md border border-white/10 p-6 rounded-3xl shadow-[0_0_30px_rgba(0,0,0,0.8)] w-full max-w-sm transform scale-95 transition-transform glass-panel">
            ...
            <div class="flex gap-2 mt-6">
                <button id="ps5-dialog-btn-cancel" class="flex-1 bg-white/5 hover:bg-white/10 text-[var(--text-muted)] rounded-xl py-3.5 font-black text-[10px] tracking-widest uppercase transition-all hidden" data-i18n="modal_cancel">CANCELAR</button>
    ...
    <div id="lightbox-modal" class="fixed inset-0 z-[300] bg-black/95 hidden flex flex-col transition-opacity opacity-0">
        <div class="flex justify-between items-center p-4 absolute top-0 left-0 w-full z-10 bg-gradient-to-b from-black/80 to-transparent">
            <span id="lightbox-title" class="text-[10px] font-mono text-white/70 truncate pr-4 drop-shadow-md"></span>
            <button onclick="cerrarLightbox()" class="w-10 h-10 rounded-full bg-white/10 text-white flex items-center justify-center hover:bg-red-500 transition-colors backdrop-blur-md border border-white/10"><i class="fa-solid fa-xmark text-lg"></i></button>
        </div>
        ...
        function setLanguage(lang) { 
            ...
            if(lang === 'es') { if(btnEs) { btnEs.style.backgroundColor = 'var(--theme-prim)'; btnEs.style.color = 'black'; btnEs.style.boxShadow = '0 0 10px color-mix(in srgb, var(--theme-prim) 40%, transparent)'; } if(btnEn) { btnEn.style.backgroundColor = 'transparent'; btnEn.style.color = 'rgba(255,255,255,0.5)'; btnEn.style.boxShadow = 'none'; } } 
            else { if(btnEn) { btnEn.style.backgroundColor = 'var(--theme-prim)'; btnEn.style.color = 'black'; btnEn.style.boxShadow = '0 0 10px color-mix(in srgb, var(--theme-prim) 40%, transparent)'; } if(btnEs) { btnEs.style.backgroundColor = 'transparent'; btnEs.style.color = 'rgba(255,255,255,0.5)'; btnEs.style.boxShadow = 'none'; } } 
        ...
        let esBovedaGlobal = false; // Nueva variable para saber en qué modo de capturas estamos
        ...
        async function abrirOpciones(titulo, cusa, imgSrc, version) {
            ...
            // Ocultar badge de capturas por defecto
            ...
            // Traer Info y Chequear cantidad de capturas en paralelo
        ...
        function crearBotonCarpeta(newPath, autoSelect = false) {
            ...
            btn.type = 'button';
            btn.className = 'folder-btn w-full h-full btn-ps5 rounded-xl py-3 px-2 text-[11px] font-bold text-white/60 border border-white/10 transition-colors truncate';
        ...
        async function eliminarCarpetaPersonalizada(path, divElement) {
            const seguro = await ps5Confirm(t('j_del_route'), t('j_del_route_m') + `<br><b class="text-white mt-1 block">${path}</b>`, "fa-trash", "bg-red-600 text-white border border-red-500/50");
            if(!seguro) return;
        ...
        function updateFileName(input) {
            ...
            if (input.files && input.files.length > 0) {
                ...
                display.classList.replace('text-white/60', 'text-[var(--text-main)]');
                ...
                display.classList.replace('text-[var(--text-main)]', 'text-white/60');
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    let htmlControls = `<div class="col-span-full flex justify-end gap-2 mb-2">
                        <button onclick="actualizarIpCelular()" class="bg-yellow-600/20 text-yellow-400 hover:bg-yellow-600 hover:text-white px-3 py-1.5 rounded-lg text-[9px] font-black tracking-widest transition-colors active:scale-95 border border-yellow-500/30"><i class="fa-solid fa-pen mr-1"></i> EDITAR IP</button>
                        <button onclick="renderRpiList()" class="hover:text-black px-3 py-1.5 rounded-lg text-[9px] font-black tracking-widest transition-colors active:scale-95 border hover:bg-white" style="color: var(--theme-prim); border-color: var(--theme-prim);"><i class="fa-solid fa-rotate mr-1"></i> REFRESCAR</button>
                    </div>`;
                    
                    let htmlCards = '';
                    rpiRawList.forEach((pkg, idx) => {
                        let idCard = 'rpi-card-' + idx;
                        let origenHtml = pkg.origen ? `<span class="text-[8px] font-mono tracking-widest bg-white/10 px-1 py-0.5 rounded border border-white/5 text-white">${pkg.origen}</span>` : '';
                        htmlCards += `
                        <div id="${idCard}" class="rpi-card rounded-2xl p-2 aspect-square flex flex-col relative group bg-black/40 border border-white/5" onclick="selectRpiPkg('${pkg.path || pkg.nombre}', '${idCard}')">
                            <div class="rpi-card-check"><i class="fa-solid fa-check"></i></div>
                            <div class="w-full flex-1 rounded-xl bg-black/60 border border-white/5 flex items-center justify-center overflow-hidden mb-2 relative shadow-inner">
                            ...
                            <div class="px-1 flex flex-col items-center text-center w-full overflow-hidden">
                                <div id="title-container-${idCard}" class="w-full mb-1">
                                    <h3 class="text-[10px] font-black text-[var(--text-main)] leading-tight truncate w-full uppercase">Cargando...</h3>
                                </div>
                                <div class="flex items-center justify-center w-full gap-1 mb-1">
                                    ${origenHtml}
                                </div>
                                <div class="flex items-center gap-1 justify-center w-full">
                                    <span id="cusa-${idCard}" class="text-[8px] font-mono font-bold tracking-widest bg-black/60 px-1 py-0.5 rounded border truncate" style="color: var(--theme-prim); border-color: var(--theme-prim);">----</span>
                                    <span class="text-[8px] text-[var(--text-muted)] font-mono tracking-widest">${pkg.size_fmt}</span>
                                </div>
                            </div>
                        </div>`;
                    });
                ...
                if (data.status === 'success') {
                    ...
                    if (titleContainer) {
                        ...
                        if (safeTitle.length > 15) {
                            titleContainer.innerHTML = `<marquee behavior="alternate" scrollamount="1"><h3 class="text-[10px] font-black text-[var(--text-main)] leading-tight inline uppercase">${safeTitle}</h3></marquee>`;
                        } else {
                            titleContainer.innerHTML = `<h3 class="text-[10px] font-black text-[var(--text-main)] leading-tight truncate uppercase">${safeTitle}</h3>`;
                        }
                    }
        ...
        async function iniciarColaInstalacionRPI() {
            ...
            for (let i = 0; i < rpiQueue.length; i++) {
                let selectedPath = rpiQueue[i]; // ej: servidor_rpi/juego.pkg o microsd/pkgs_rpi/juego.pkg
                ...
                let displayName = pathParts[pathParts.length - 1]; // solo el nombre para la pantalla
        ...
        // ==========================================
        // 13. EXPLORADOR FTP Y SELECTOR DE CARPETAS
        // ==========================================
        ...
        // --- LÓGICA DEL MINI EXPLORADOR DE CARPETAS LOCAL ---
        let dirActualExplorador = '/storage/emulated/0';
        function abrirExploradorIconos() { document.getElementById('explorador-ui').classList.remove('hidden'); cargarCarpetasLocal(dirActualExplorador); }
        function cerrarExploradorIconos() { document.getElementById('explorador-ui').classList.add('hidden'); }
        async function cargarCarpetasLocal(ruta) {
            const lista = document.getElementById('lista-carpetas'); lista.innerHTML = '<p class="text-center text-white/50 py-4"><i class="fa-solid fa-spinner fa-spin mr-2"></i>Leyendo memoria...</p>';
            try {
                const res = await fetch(`api_explorador.php?action=list&dir=${encodeURIComponent(ruta)}`);
                const data = await res.json();
                dirActualExplorador = data.current;
                document.getElementById('ruta-actual-explorador').innerHTML = `Ruta:<br><span class="text-white">${data.current}</span>`;
                let html = '';
                if (data.current !== '/storage/emulated/0') { html += `<div onclick="cargarCarpetasLocal('${data.parent}')" class="p-3 bg-white/5 rounded-xl hover:bg-white/10 cursor-pointer flex items-center transition-colors border border-transparent hover:border-white/10"><i class="fa-solid fa-level-up-alt mr-3 text-yellow-400"></i> <span class="font-bold tracking-wide">Atrás</span></div>`; }
                if (data.folders.length === 0) { html += `<p class="text-center text-white/30 text-xs py-4">Esta carpeta está vacía</p>`; } 
                else { data.folders.forEach(folder => { html += `<div onclick="cargarCarpetasLocal('${data.current}/${folder}')" class="p-3 bg-white/5 rounded-xl hover:bg-white/10 cursor-pointer flex items-center transition-colors border border-transparent hover:border-white/10"><i class="fa-solid fa-folder mr-3 text-blue-300"></i> <span class="tracking-wide">${folder}</span></div>`; }); }
                lista.innerHTML = html;
            } catch (error) { lista.innerHTML = '<p class="text-red-400 text-xs text-center py-4 bg-red-500/10 rounded-lg">Error al leer la memoria. ¿Diste permisos en Termux?</p>'; }
        }
        async function guardarRutaIconos() {
            try {
                const res = await fetch('api_explorador.php?action=save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path: dirActualExplorador, type: 'icons' }) });
                const data = await res.json();
                if (data.success) { 
                    document.getElementById('ruta-actual-display').innerText = dirActualExplorador;
                    cerrarExploradorIconos(); ps5Notification("CONFIGURACIÓN", "Ruta de íconos guardada.", "fa-folder-check");
                    actualizarGaleria();
                }
            } catch (error) { ps5Alert("ERROR", "No se pudo guardar la ruta."); }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Cargar ruta de iconos guardada
            fetch('config.json').then(r => r.json()).then(config => { if(config.icons_path) document.getElementById('ruta-actual-display').innerText = config.icons_path; }).catch(() => {});

            renderShortcuts(); actualizarGaleria(); actualizarPayloads(); renderCategorias(); cargarBibliotecaLocal(); AudioEngine.loadSettings();
            let blurSlider = document.getElementById('blur-slider'); if (blurSlider) { blurSlider.addEventListener('input', (e) => updateWallpaperBlur(e.target.value)); blurSlider.addEventListener('change', (e) => updateWallpaperBlur(e.target.value)); }
            let partSlider = document.getElementById('particles-slider'); if (partSlider) { partSlider.addEventListener('change', (e) => updateParticlesCount(e.target.value)); }
        });
    </script>
</body>
</html>
